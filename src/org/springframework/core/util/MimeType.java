...
import org.springframework.core.util.MimeTypeUtils;
...
public class MimeType implements Comparable<MimeType>,Serializable {
   ...
   public MimeType(String type){this(type,"*");}

   public MimeType(String type, String subtype){this(type,subtype, Collections.emptyMap());}

   public MimeType(String type, String subtype, Charset charSet){
       this(type,subtype, Collections.singletonMap("chaeset",charSet.name()));
   }
   ...
   private boolean isQuotedString(String s){
        return s.length() < 2? false : s.startsWith("\"") && s.endsWith("\"") || s.startsWith("'") && s.endsWith("'");
   }

   protected String unquote (String s){
       return s == null ? null:(this.isQuotedString(s)?s.substring(1,s.length()-1):s);
   }

   public boolean isWildcardType(){return "*".equals(this.getType());}
   public boolean isWildcardSubtype(){return "*".equals(this.getSubtype()) || this.getSubtype().startsWith("*+");}
   public boolean isConcrete(){return !this.isWildcardType() && !this.isWildcardSubtype();}
   ...
   public String getParameter(String name){return (String)this.parameters.get(name);}
   ...
   public static class SpecificityComparator<T extends MimeType> implements Comparator<T>{
       ...
       public int compare(T mimeType1, T mimeType2) {
           return mimeType1.isWildcardType() && !mimeType2.isWildcardType()?1:(mimeType2.isWildcardType() && !mimeType1.isWildcardType()?-1:(!mimeType1.getType().equals(mimeType2.getType())?0:(mimeType1.isWildcardSubtype() && !mimeType2.isWildcardSubtype()?1:(mimeType2.isWildcardSubtype() && !mimeType1.isWildcardSubtype()?-1:(!mimeType1.getSubtype().equals(mimeType2.getSubtype())?0:this.compareParameters(mimeType1, mimeType2))))));
       }
