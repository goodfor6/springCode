...
public  class SocketUtils {
	...
	public static int findAvailableUdpatePort(){
		return findAvailableUdpatePort(PORT_RANGE_MIN);
	}
	
	public static int findAvailableUdpatePort(int minPort){
	...
	private static enum SocketType{
		...
		int findAvailablePort(int minPort, int maxPort){
			...
			Assert.isTrue(maxPort > minPort,"'maxPort' must be greater tahn 'minPort'");
			...
			do{
				if(++searchCounter > portRange){
					throw new IllegalStateException(String.format(
							"Could not find an available %s port in the range [%d, %d] after %d attermpts",name(), minPort,maxPort,searchCounter));
				}
		...
		SortedSet<Integer>findAvailablePorts(int numRequested, int minPort, int maxPort){
			...
			if(availablePorts.size() != numRequested){
				throw new IllegalStateException(String.format("Could not find %d available %s ports in the range [%d, %d]", numRequested, name(), minPort,
					maxPort));
			}
