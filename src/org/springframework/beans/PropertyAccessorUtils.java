...
public abstract class PropertyAccessorUtils {
	...
	private static int getNestedPropertySeparatorIndex(String propertyPath,boolean last){
		...
		while(last ? i>=0:i<length){
			...
			case PropertyAccessor.PROPERTY_KEY_SUFFIX_CHAR: inKey=!inKey; break;
	...
	public static boolean matcheProperty(String registeredPath,String propertyPath){
	...
	public static String cannonicalPropertyName(String propertyName){
		...
		while(searchIndex!=-1){
			...
			if(keyStart!=-1){
				...
				if(keyEnd!=-1){
					...
					searchIndex=keyEnd+PropertyAccessor.PROPERTY_KEY_PREFIX.length();
				}
			}
		}
		return sb.toString();
	...
	public static String[] canonicalPropertyNames(String[]propertyNames){
		...
		for(int i=0;i<propertyNames.length;i++){
			result[i]=cannonicalPropertyName(propertyNames[i]);
		}
		return result;
	}
